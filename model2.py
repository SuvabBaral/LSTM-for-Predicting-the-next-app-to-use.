...
idx = (-predicted_app).argsort() #index of the highest values.
idx=pd.DataFrame(idx)
idx2=label_encoder_app.inverse_transform(idx.iloc[:,0:1])
idx2=pd.DataFrame(data=idx2)
test_set2=label_encoder_app.inverse_transform(test_set)
test_set2=pd.DataFrame(data=test_set2)

for i in range(1,36):
    idx3=label_encoder_app.inverse_transform(idx.iloc[:,i:i+1])
    idx3=pd.DataFrame(data=idx3)    
    idx2=pd.concat([idx2,idx3],ignore_index=True,axis=i)
